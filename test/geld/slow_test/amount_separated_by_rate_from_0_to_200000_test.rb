...
class Geld::ExciseTest < Minitest::Test
  def test_amount_separated_by_rateに該当する消費税が1つとなる期間と、1から20万までのamountを入れた際に分割された結果の合計がamountと等しいこと
    0.upto(200_000).each do |amount|
  ...
  def test_amount_separated_by_rateに該当する消費税が2つとなる期間と、1から20万までのamountを入れた際に分割された結果の合計がamountと等しいこと
    0.upto(200_000).each do |amount|
  ...
  def test_amount_separated_by_rateに該当する消費税が3つとなる期間と、1から20万までのamountを入れた際に分割された結果の合計がamountと等しいこと
    0.upto(200_000).each do |amount|
  ...
  def test_amount_separated_by_rateに該当する消費税が4つとなる期間と、1から20万までのamountを入れた際に分割された結果の合計がamountと等しいこと
    0.upto(200_000).each do |amount|
  ...
  def test_amount_separated_by_rateに該当する消費税が5つとなる期間と、1から20万までのamountを入れた際に分割された結果の合計がamountと等しいこと
    0.upto(200_000).each do |amount|
